...
- [ ] `partially_received` is valid on BOTH `client_invoices` AND `client_milestones`
      (clients pay invoices in tranches — partial payment sets status to `partially_received`)
...
- [x] Formula: `SUM(client_invoices.received_amount)` filtered by `receipt_date` year
...
**Historical bars**
- [x] Cash In: `SUM(client_invoices.received_amount)` by `receipt_date` month ✅
- [x] Cash Out: `SUM(payment_vouchers.net_paid)` by `voucher_date` month ✅
...
6. Search entire codebase for `partially_received` on `client_invoices` → this IS valid (partial client payments). Only flag if used as a filter that EXCLUDES partially paid invoices from receivables totals.
7. Search entire codebase for `cash_receipts` → must return zero matches
8. Search entire codebase for `invoice_date` used in cash-received bucketing → must return zero matches
   (NOTE: `receipt_date` is the CORRECT field for cash-in — do NOT flag it)
