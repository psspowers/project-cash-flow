...
> ⚠️ IMPORTANT CORRECTION — April 2026
> Two formulas in this section were wrong in the original document and have been corrected.
> See "Monthly Cash In" and "Monthly Cash Out" below for details.
> The Master Architect Brief is the authoritative source where conflicts exist.
...
**Formula:** `SUM(client_invoices.received_amount) WHERE receipt_date LIKE 'YYYY-MM%'`

> ⚠️ CORRECTED April 2026 — original said `invoice_date`. This is wrong.
> `invoice_date` is NULL on 228 of 268 EPC invoices — using it returns zero for all months.
> `receipt_date` is the bank-cleared date and is the only correct field for cash-in bucketing.
> This was a confirmed production bug fixed across 7 files.

---

### Monthly Cash Out (historical)
**What it means:** Actual supplier payments made in a specific month.
**Formula:** `SUM(payment_vouchers.net_paid) WHERE voucher_date LIKE 'YYYY-MM%' AND status = 'issued'`

> ⚠️ CORRECTED April 2026 — original said `vendor_invoices.received_amount` by `invoice_date`.
> Both the table and the date field were wrong:
> - `vendor_invoices.received_amount` is what the supplier invoiced us, not what PSS paid.
>   It is systematically ฿56M higher than actual bank disbursements due to WHT deductions.
> - `invoice_date` is when the supplier raised their invoice — not when PSS paid.
> Correct source: `payment_vouchers.net_paid` by `voucher_date` where `status = 'issued'`.
> This is the only table that records actual bank outflows.
...
| Shade | Meaning | Data source |
|---|---|---|
| Dark solid red | Actually paid — cash out of bank | `payment_vouchers.net_paid` WHERE `status = 'issued'` |
...
**✅ `partially_received` IS valid on `client_invoices`**
Clients frequently pay invoices in tranches. When an accounts user records a partial
payment, the system sets `status = partially_received`. When the full invoice amount
is paid, status becomes `received`.
Valid status values: `pending` | `partially_received` | `received`
...
**❌ Using `invoice_date` for cash-in bucketing on `client_invoices`**
`invoice_date` is NULL on the majority of EPC invoices imported from bank statement data.
Using it for monthly cash-in calculations returns zero for every month.
The correct field is `receipt_date` — the date the bank payment cleared.

**❌ Using `vendor_invoices.received_amount` for historical cash-out**
This field is ฿56M higher than actual bank disbursements because it includes WHT retained by PSS.
The correct source for actual cash paid to suppliers is `payment_vouchers.net_paid` by `voucher_date`.
