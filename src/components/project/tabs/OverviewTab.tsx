...
export default function OverviewTab() {
  ...
  const revenueForecastData = useMemo(() => {
    ...
    clientInvoices.forEach(inv => {
      if (inv.receipt_date) allMonths.add((inv.receipt_date as string).substring(0, 7));
      // Include invoice_date for pending invoices so their scatter dots have an X-axis slot
      if (inv.status === 'pending' && inv.invoice_date) allMonths.add(inv.invoice_date.substring(0, 7));
    });
