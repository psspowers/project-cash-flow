...
export default function VarianceTab() {
  const { estimation, budget, orders, vos, totalReceived, totalPaid, voTotalCost } = useProjectDetail();

  const actualByCategory = (cat: typeof COSTING_CATEGORY_KEYS[number]): number => {
    const mapped = CATEGORY_MAP[cat];
    return orders.filter(o => o.cost_category === mapped).reduce((s, o) => s + o.po_amount_excl_vat, 0);
  };
  ...
  const totalActual = COSTING_CATEGORY_KEYS.reduce((s, k) => s + actualByCategory(k), 0);
  const totalVar = totalBud - totalActual;
  ...
  const forecastMargin = (budgetRevenue + voRevAdj) - (totalBud + voCostAdj) - Math.max(0, totalActual - totalBud);
      ...
      <table className="w-full text-xs">
        ...
        <tbody>
          {COSTING_CATEGORY_KEYS.map(k => {
            ...
            const actual = actualByCategory(k);
            const variance = bud - actual;
            const variancePct = bud > 0 ? (variance / bud) * 100 : 0;
            const cellCls = actual > bud
            ...
            return (
              <tr key={k} className="border-b border-[rgba(0,0,0,0.04)] hover:bg-[#F8F8F7]">
                ...
                <td className={`px-4 py-2.5 font-medium ${cellCls}`}>{fmtTHB(actual)}</td>
          ...
          <tr className="bg-[#F8F8F7] border-t-2 border-[rgba(0,0,0,0.1)] font-semibold">
            ...
            <td className="px-4 py-3">{fmtTHB(totalActual)}</td>
