...
export function useProjectData(id: string | undefined): ProjectData {
  ...
  const reload = useCallback(async () => {
    ...
    const [projRes, costRes, voRes, poRes, vendorRes, profilesRes, cmRes, ciRes, pvRes, pmRes] = await Promise.all([
      ...
      supabase.from('po_milestones').select('*, purchase_orders!inner(project_id)').eq('purchase_orders.project_id', id),
    ]);
  ...
  return {
    ...
    clientMilestones, clientInvoices, poMilestones, paymentVouchers, vendors, profiles,
